...
class PeakTest extends TestCase {
    ...
    public function testHasNoValidConfiguration(): void {
        $json = file_get_contents(__DIR__ . '/responses/getPeakCalculationResult.json');
        ...
        $this->api->expects($this->once())
            ->method('run')
            ->with(
                $this->identicalTo('tickets/457564/yield-losses/peak'),
                $this->identicalToUrl(
        ...
        $calculationResult = $this->api->ticket(457564)->yieldLosses()->peak()->get($criteria);
